...
public class LonelyTwitterActivityTest extends ActivityInstrumentationTestCase2 {
    ...
    public void testEditATweet() {
        ...
        // assert that the tweet being shown on the edit screen is tweet we clicked on
        assertEquals(Tweet.getText, "hamburgers");

        // edit the text of that tweet
        activity.runOnUiThread(new Runnable() {
            public void run() {
                bodyText.setText("Old hamburgers");
            }
        });
        getInstrumentation().waitForIdleSync();

        // save our edits
        Tweet tweet1 = new NormalTweet();
        tweet1.saveInFile();

        // assert edits were saved into tweet correctly


        // assert that our edits are shown on the screen to user back in main activity

        // end of test: clear the data
        // end of test: make sure edit activity is closed
